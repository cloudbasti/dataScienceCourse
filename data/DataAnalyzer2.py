...
class DataAnalyzer:
    ...
    def analyze_temporal_outliers(self) -> Dict:
        """
        Analyze outliers in the context of temporal patterns and special days.
        """
        ...
        df_temp['month'] = df_temp['Datum'].dt.month
        df_temp['day_of_week'] = df_temp['Datum'].dt.dayofweek
        df_temp['is_weekend'] = df_temp['day_of_week'].isin([5, 6]).astype(int)
        
        # Calculate outliers using different methods
        z_scores = np.abs(stats.zscore(df_temp['Umsatz']))
        df_temp['is_zscore_outlier'] = z_scores > 3
        
        Q1 = df_temp['Umsatz'].quantile(0.25)
        Q3 = df_temp['Umsatz'].quantile(0.75)
        IQR = Q3 - Q1
        df_temp['is_iqr_outlier'] = (df_temp['Umsatz'] < (Q1 - 1.5 * IQR)) | (df_temp['Umsatz'] > (Q3 + 1.5 * IQR))
        
        # Analyze outliers by different dimensions
        analysis = {
            'monthly_patterns': {},
        ...
        # Monthly patterns of outliers
        monthly_stats = df_temp.groupby('month').agg({
            'Umsatz': ['mean', 'std', 'count'],
            'is_zscore_outlier': 'sum',
            'is_iqr_outlier': 'sum'
        }).round(2)
        analysis['monthly_patterns'] = monthly_stats.to_dict()
        
        # Day of week patterns
        dow_stats = df_temp.groupby('day_of_week').agg({
            'Umsatz': ['mean', 'std', 'count'],
            'is_zscore_outlier': 'sum',
            'is_iqr_outlier': 'sum'
        }).round(2)
        analysis['day_of_week_patterns'] = dow_stats.to_dict()
        ...
        analysis['weekend_effect'] = weekend_stats.to_dict()
        
        # Top outliers analysis
        top_outliers = df_temp[df_temp['is_iqr_outlier']].nlargest(100, 'Umsatz')[
            ['Datum', 'Umsatz', 'month', 'day_of_week', 'is_weekend']
        ].to_dict('records')
        
        analysis['top_outliers'] = top_outliers
...
def main():
    ...
    temporal_analysis = analyzer.analyze_temporal_outliers()
    print("\nTemporal Analysis of Outliers:")
    print("\nMonthly Patterns:")
    print(pd.DataFrame(temporal_analysis['monthly_patterns']))
    print("\nDay of Week Patterns:")
    print(pd.DataFrame(temporal_analysis['day_of_week_patterns']))
    print("\nTop 10 Highest Turnover Days:")
    for outlier in temporal_analysis['top_outliers']:
        print(f"Date: {outlier['Datum']}, Turnover: {outlier['Umsatz']:.2f}")
