...
# NEW FUNCTION: Exportable function to prepare test data with imputation
def prepare_test_data_with_imputation(input_path=None, output_path=None):
    """
    Prepare test data by applying weather imputation and save to specified path.
    
    Parameters:
    -----------
    input_path : str, optional
        Path to the raw test data CSV file.
        Default is "data/SubmissionPreparation/prepared_test_data.csv"
    output_path : str, optional
        Path where to save the imputed test data.
        Default is "data/SubmissionPreparation/test_data_final_after_imputation.csv"
        
    Returns:
    --------
    str
        Path to the saved imputed test data file
    """
    # Use default paths if not provided
    if input_path is None:
        input_path = "data/SubmissionPreparation/prepared_test_data.csv"
    
    if output_path is None:
        output_path = "data/SubmissionPreparation/test_data_final_after_imputation.csv"
    ...
    print(f"Saved imputed test data to: {output_path}")
...
def main():
    ...
    df = pd.read_csv("data/SubmissionPreparation/prepared_test_data.csv")
    df['Datum'] = pd.to_datetime(df['Datum'])
